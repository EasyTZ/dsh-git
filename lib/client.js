window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// createPortal：面板要挂到 document.body 才能盖过桌面端标题栏，见 .dsgBackdrop
		// 的注释。react-dom 是 dsh 前端的 seed 模块（内置弹窗自己也 require 它），
		// 不需要额外声明依赖。
		let react_dom = require("react-dom");

		const NS = "git";

		// dsh 的外部 store 方法不是稳定的裸函数契约：新版实现会通过 `this`
		// 访问内部快照。React 会脱离对象调用 subscribe/getSnapshot，所以这里统一
		// 包一层、保留接收者；同时用 useCallback 保持订阅函数跨渲染稳定。
		...
		const zh = {
			...
			"git.workspace.missing": "工作区目录不存在，可能已被移动或删除",
			"git.repo.label": "仓库",
			"git.repo.none": "工作区里没有找到 Git 仓库（只扫描了一层子目录）",
			"git.repo.gone": "这个子仓库不在了，请重新选择",
			...
			"git.detail.title": "提交详情",
			"git.detail.close": "关闭",
			"git.detail.loading": "正在读取提交详情…",
			"git.detail.retry": "重试",
			"git.detail.message": "提交信息",
			"git.detail.files": "改动文件",
			"git.detail.noFiles": "这个提交没有文件改动",
			"git.detail.copy": "复制完整 hash",
			"git.detail.copied": "已复制",
			"git.detail.binary": "二进制",
			"git.detail.summary": "{files} 个文件",
			"git.detail.openHint": "查看提交详情",
			"git.detail.status.A": "新增",
			"git.detail.status.M": "修改",
			"git.detail.status.D": "删除",
			"git.detail.status.R": "重命名",
			"git.detail.status.C": "复制",
			"git.detail.status.T": "类型变更",
		...
		const en = {
			...
			"git.workspace.missing": "The workspace directory is gone — it may have been moved or deleted",
			"git.repo.label": "Repository",
			"git.repo.none": "No Git repository found in this workspace (only one level down is scanned)",
			"git.repo.gone": "That repository is gone — pick another one",
			...
			"git.detail.title": "Commit details",
			"git.detail.close": "Close",
			"git.detail.loading": "Loading commit details…",
			"git.detail.retry": "Retry",
			"git.detail.message": "Message",
			"git.detail.files": "Changed files",
			"git.detail.noFiles": "This commit changes no files",
			"git.detail.copy": "Copy full hash",
			"git.detail.copied": "Copied",
			"git.detail.binary": "binary",
			"git.detail.summary": "{files} file(s)",
			"git.detail.openHint": "View commit details",
			"git.detail.status.A": "added",
			"git.detail.status.M": "modified",
			"git.detail.status.D": "deleted",
			"git.detail.status.R": "renamed",
			"git.detail.status.C": "copied",
			"git.detail.status.T": "type changed",
		...
		const css = [
			...
			// footer action 都是 width:100% 的按钮 —— 两个以上插件同时注册就被挤成同行
			// 的半宽按钮，文字全被省略号吃掉。改成纵向，一个 action 独占一行。
			//
			// **这条规则在四个 footer 插件里各写一份，是有意的重复**：原先只有终端面板
			// 写了它，于是装了终端面板的机器一切正常，只装市场 + 余额的机器上图标挤成
			// 一行 —— 一个插件的样式在替别的插件兜底，这是隐性依赖。任何一个插件都可能
			// 被单独安装，所以每个往这个槽里放东西的插件都得自带这条。声明完全相同，
			// 重复注入无副作用。
			//
			...
			// 背景遮罩：点它关面板（类似对话框点空白处关闭）。
			//
			// z-index 必须站到 1000 这一档 —— 这是 dsh 内置弹窗遮罩用的层级，不是随手
			// 取的大数。桌面端（dsDesktop）的自绘标题栏 `#dsh-titlebar` 是 z-index:900，
			// 它最左段专门用 --dsw-specific-sidebar-fill 跟侧边栏拼同一种底色。我们原来
			// 挂在 shell.overlay 里（那层是 layout 包的 .overlayLayer{z-index:20}），
			// 遮罩最多只能压到 20，压不过 900 的标题栏：结果是侧边栏被压暗、标题栏左段
			// 纹丝不动，接缝处直接露出色差。所以面板与遮罩一起 createPortal 到
			// document.body（见 GitPanel），跳出 .overlayLayer 的层叠上下文，再用
			// 1000/1001 跟内置弹窗同层——标题栏会跟着一起变暗。
			//
			// 代价与内置弹窗一致：面板开着时标题栏被盖住，窗口按钮点不到、也拖不动窗口。
			// 关闭途径有 Esc 与点遮罩两条，够用。
			".dsgBackdrop{position:fixed;inset:0;z-index:1000;background:var(--dsw-alias-bg-mask-1,rgba(0,0,0,.32));opacity:0;pointer-events:none;transition:opacity .16s ease}",
			...
			// 背板用 specific-sidebar-fill（深色 #1b1b1c / 浅色 #f9fafb）—— 和左侧菜单同底，
			// 面板读起来是「应用的一部分」。**别用 bg-overlay**：它在深色下是 #61666b 的
			// 中灰，浮在 #151517 的界面上就是一块突兀的灰板子，正是「黑白灰很丑」的来源。
			".dsgPanel{position:fixed;top:50%;right:20px;z-index:1001;width:420px;max-width:92vw;height:min(760px,88vh);display:flex;flex-direction:column;padding-bottom:32px;background:var(--dsw-specific-sidebar-fill,#1b1b1c);border-radius:14px;box-shadow:0 16px 48px rgba(0,0,0,.32);color:var(--dsw-alias-label-primary,#f9fafb);font-size:13px;overflow:hidden;" +
			...
			".dsgBodyFade{position:absolute;left:0;right:0;bottom:32px;height:18px;pointer-events:none;background:linear-gradient(to bottom,transparent,var(--dsw-specific-sidebar-fill,#1b1b1c))}",
			...
			".dsgDropdownTrigger{display:flex;align-items:center;justify-content:space-between;gap:6px;width:100%;height:30px;padding:0 8px;border-radius:7px;border:1px solid var(--dsw-alias-border-l1,rgba(255,255,255,.08));background:var(--dsw-specific-input-major,#2c2c2e);color:var(--dsw-alias-label-primary,#f9fafb);font-size:12.5px;font-family:inherit;cursor:pointer;text-align:left}",
			...
			// 满宽下拉克制，但一眼能看出可点。
			".dsgDropdownCompact .dsgDropdownTrigger{width:auto;max-width:240px;height:26px;padding:0 8px;border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.12));background:var(--dsw-specific-input-major,#2c2c2e);border-radius:7px}",
			...
			".dsgDropdownMenu{position:absolute;top:calc(100% + 4px);left:0;z-index:10;min-width:160px;max-width:280px;max-height:220px;overflow-y:auto;background:var(--dsw-specific-menu,#353638);border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.12));border-radius:10px;box-shadow:0 8px 24px rgba(0,0,0,.36);padding:4px}",
			...
			// 最后一块（历史区）不留下外边距：它下面已经压着 body 的 padding 和面板
			// 那 32px 底部留白，再叠 14px 就是白扔掉小半行提交的位置。
			".dsgBody > .dsgSection:last-child{margin-bottom:0}",
			...
			".dsgCommitInput{width:100%;resize:vertical;min-height:56px;padding:8px 10px;border-radius:8px;border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.12));background:var(--dsw-specific-input-major,#2c2c2e);color:var(--dsw-alias-label-primary,#f9fafb);font-size:12.5px;font-family:inherit;box-sizing:border-box}",
			...
			//    看不见了。
			// 2) 底色用 button-ghost-active-fill —— 上游正是为「次级/幽灵按钮的面」
			//    准备的 token，浅色 neutral-100、深色 neutral-750，两套主题各有值。
			//    曾经用过 transparent（深色下就是面板底色，糊成一片）与 bg-layer-2
			//    （浅色下它和面板底 bg-overlay 几乎是同一个白，看不清），于是退而
			//    求其次写死中性灰「一个值通吃两个主题」。灰确实两边都有对比，但两边
			//    都不好看——它不参与主题、永远是那块脏灰。ghost-active-fill 才是
			//    正解：它自带两套值，不需要我们写两套，也不脱离主题。
			// 3) hover 态显式重申 color，不能靠继承——一旦别处的规则改了文字色，
			//    这里就会重演「底白字白」。
			".dsgCommitBtnSecondary{background:var(--dsw-alias-button-ghost-active-fill,#2a2a2e);border:1px solid var(--dsw-alias-border-l2,rgba(255,255,255,.12));color:var(--dsw-alias-label-primary,#f9fafb)}",
			".dsgCommitBtnSecondary:hover:not(:disabled){background:var(--dsw-alias-interactive-bg-active,rgba(255,255,255,.14));border-color:var(--dsw-alias-border-l3,rgba(255,255,255,.2));color:var(--dsw-alias-label-primary,#f9fafb)}",
			...
			// 之后才生效（同特异性，后写的赢）。
			".dsgCommitBtnSecondary:disabled{opacity:1;background:transparent;border-color:var(--dsw-alias-border-l2,rgba(255,255,255,.12));color:var(--dsw-alias-label-tertiary,#8b949e);cursor:default}",
			".dsgHistoryRow{padding:6px 4px;border-radius:6px}",
			".dsgHistoryRow{cursor:pointer;transition:background .12s ease}",
			".dsgHistoryRow:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.08))}",
			".dsgHistoryRow:focus-visible{outline:2px solid var(--dsw-alias-button-primary-fill,#4d6bfe);outline-offset:-2px}",
			".dsgDetailMask{position:absolute;inset:0;z-index:20;display:flex;align-items:center;justify-content:center;padding:16px;background:var(--dsw-alias-bg-mask-1,rgba(0,0,0,.5));box-sizing:border-box}",
			".dsgDetailCard{display:flex;flex-direction:column;width:100%;max-height:100%;background:var(--dsw-alias-bg-layer-2,#2c2c2e);border:1px solid var(--dsw-alias-border-l1,rgba(255,255,255,.1));border-radius:12px;box-shadow:0 12px 32px rgba(0,0,0,.45);overflow:hidden}",
			".dsgDetailHead{flex:none;display:flex;align-items:center;gap:8px;padding:10px 12px;border-bottom:1px solid var(--dsw-alias-border-l1,rgba(255,255,255,.08))}",
			".dsgDetailTitle{flex:1;min-width:0;font-size:12.5px;font-weight:600;color:var(--dsw-alias-label-primary,#f9fafb)}",
			".dsgDetailClose{flex:none;width:24px;height:24px;border:none;border-radius:6px;background:transparent;color:var(--dsw-alias-label-secondary,#cfd3d6);font-size:15px;line-height:1;cursor:pointer}",
			".dsgDetailClose:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.08));color:var(--dsw-alias-label-primary,#f9fafb)}",
			".dsgDetailBody{flex:1;min-height:0;overflow-y:auto;padding:12px}",
			".dsgDetailMetaRow{display:flex;flex-wrap:wrap;align-items:center;gap:6px;font-size:11.5px;color:var(--dsw-alias-label-tertiary,#8b949e);margin-bottom:10px}",
			".dsgDetailHash{font-family:ui-monospace,monospace;color:var(--dsw-alias-label-secondary,#cfd3d6)}",
			".dsgDetailCopy{border:1px solid var(--dsw-alias-border-l1,rgba(255,255,255,.14));border-radius:5px;background:transparent;color:var(--dsw-alias-label-tertiary,#8b949e);font-size:10.5px;font-family:inherit;padding:1px 6px;cursor:pointer}",
			".dsgDetailCopy:hover{color:var(--dsw-alias-label-primary,#f9fafb)}",
						// 底色用 interactive-bg-hover 这类**相对色调**（浅色是极淡的深、深色是极淡
			// 的白），不用 bg-layer-2：后者在浅色主题下是纯白，和同为白的卡片底完全
			// 分不开，等于没有底——面板里那个次级按钮当年就栽在这上面。再加一道
			// border-l1 兜底，即使色调差被主题调没了，边界仍在。
			".dsgDetailMsg{white-space:pre-wrap;overflow-wrap:anywhere;font-size:12.5px;line-height:1.65;color:var(--dsw-alias-label-primary,#f9fafb);background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.06));border:1px solid var(--dsw-alias-border-l1,rgba(255,255,255,.08));border-radius:8px;padding:9px 11px;margin-bottom:12px}",
			".dsgDetailSubTitle{display:flex;align-items:baseline;gap:8px;font-size:11.5px;font-weight:600;color:var(--dsw-alias-label-secondary,#cfd3d6);margin-bottom:6px}",
			".dsgDetailSubCount{font-weight:400;color:var(--dsw-alias-label-tertiary,#8b949e)}",
			".dsgDetailFiles{list-style:none;margin:0;padding:0;display:flex;flex-direction:column;gap:2px}",
			".dsgDetailFile{display:flex;align-items:center;gap:8px;padding:4px 6px;border-radius:6px;font-size:12px}",
			".dsgDetailFile:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(255,255,255,.08))}",
			".dsgDetailBadge{flex:none;width:16px;height:16px;border-radius:4px;display:inline-flex;align-items:center;justify-content:center;font-family:ui-monospace,monospace;font-size:10px;font-weight:700}",
			// 徽标底色用 color-mix 从**文字色本身**兑出 16% 的淡底：底与字必然同源、
			// 自动跟着主题走，也不用赌上游给某个 state 定义了 tertiary —— 实测
			// state-error 就只有 primary/secondary，没有 tertiary，四个徽标各挑各的
			// token 会立刻不一致。第二个参数是 color-mix 不可用时的兜底。
			".dsgDetailBadge{background:rgba(128,128,128,.16)}",
			".dsgBadgeA{color:var(--dsw-alias-state-success-primary,#3fb950);background:color-mix(in srgb,var(--dsw-alias-state-success-primary,#3fb950) 16%,transparent)}",
			".dsgBadgeM{color:var(--dsw-alias-state-warn-primary,#d29922);background:color-mix(in srgb,var(--dsw-alias-state-warn-primary,#d29922) 16%,transparent)}",
			".dsgBadgeD{color:var(--dsw-alias-state-error-primary,#f0617a);background:color-mix(in srgb,var(--dsw-alias-state-error-primary,#f0617a) 16%,transparent)}",
			".dsgBadgeR{color:var(--dsw-alias-state-business-primary,#58a6ff);background:color-mix(in srgb,var(--dsw-alias-state-business-primary,#58a6ff) 16%,transparent)}",
			".dsgDetailPath{flex:1;min-width:0;overflow-wrap:anywhere;color:var(--dsw-alias-label-primary,#f9fafb)}",
			".dsgDetailFrom{color:var(--dsw-alias-label-tertiary,#8b949e)}",
			".dsgDetailStat{flex:none;font-family:ui-monospace,monospace;font-size:11px;white-space:nowrap}",
			".dsgStatAdd{color:var(--dsw-alias-state-success-primary,#3fb950)}",
			".dsgStatDel{color:var(--dsw-alias-state-error-primary,#f0617a)}",
			".dsgDetailState{padding:14px 4px;font-size:12px;color:var(--dsw-alias-label-tertiary,#8b949e);text-align:center}",
		...
		// 每个工作区各记一个「上次选的仓库」。key 里带 workspaceId：同一个用户手里
		// 好几个多仓库工作区是常态，共用一个 key 会互相覆盖。读写都包 try —— 隐私
		// 模式下 localStorage 是会抛的，为了记个偏好把面板整个搞崩不值得。
		const REPO_KEY_PREFIX = "dsg:repo:";
		function readStoredRepo(workspaceId) {
			try { return localStorage.getItem(REPO_KEY_PREFIX + workspaceId); } catch { return null; }
		}
		function storeRepo(workspaceId, repoId) {
			try {
				if (repoId) localStorage.setItem(REPO_KEY_PREFIX + workspaceId, repoId);
				else localStorage.removeItem(REPO_KEY_PREFIX + workspaceId);
			} catch { /* 忽略 */ }
		}
		/**
		 * 决定进面板时默认停在哪个仓库：上次选的 → 服务端建议的（.git 最近被动过的
		 * 那个）→ 列表第一个。上次选的要先确认它还在列表里，否则删掉一个子仓库之后
		 * 会一直停在一个不存在的选项上。
		 */
		function pickRepo(workspaceId, repos, suggested) {
			if (repos.length === 0) return null;
			const has = (id) => repos.some((repo) => repo.id === id);
			const stored = readStoredRepo(workspaceId);
			if (stored && has(stored)) return stored;
			if (suggested && has(suggested)) return suggested;
			return repos[0].id;
		}
		...
		function errorMessage(t, error) {
			...
			switch (error.code) {
				...
				case "workspace-missing": return t("git.workspace.missing");
				case "repo-not-found": return t("git.repo.gone");
		...
		/** 状态字母 → 徽标配色。未知状态退回 M 的中性色，不至于没有样式。 */
		const STATUS_CLASS = { A: "dsgBadgeA", M: "dsgBadgeM", D: "dsgBadgeD", R: "dsgBadgeR", C: "dsgBadgeR", T: "dsgBadgeM" };

		function CommitFileRow({ t, file }) {
			const badge = STATUS_CLASS[file.status] ?? "dsgBadgeM";
			const statusLabel = t("git.detail.status." + file.status);
			return react_jsx_runtime.jsxs("li", { className: "dsgDetailFile", children: [
				react_jsx_runtime.jsx("span", {
					className: "dsgDetailBadge " + badge,
					title: statusLabel,
					"aria-label": statusLabel,
					children: file.status
				}),
				react_jsx_runtime.jsxs("span", { className: "dsgDetailPath", title: file.path, children: [
					// 重命名要把「从哪来」也显示出来，否则只看到新名字，等于丢了这次改动的一半信息。
					file.from ? react_jsx_runtime.jsxs("span", { className: "dsgDetailFrom", children: [file.from, " → "] }) : null,
					file.path
				] }),
				react_jsx_runtime.jsx("span", { className: "dsgDetailStat", children:
					file.binary
						? react_jsx_runtime.jsx("span", { className: "dsgDetailFrom", children: t("git.detail.binary") })
						: react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, { children: [
							react_jsx_runtime.jsx("span", { className: "dsgStatAdd", children: "+" + (file.insertions ?? 0) }),
							" ",
							react_jsx_runtime.jsx("span", { className: "dsgStatDel", children: "−" + (file.deletions ?? 0) })
						] })
				})
			] });
		}
		...
		/**
		 * 提交详情弹窗。列表行里只放得下 subject 一行，正文与文件清单都看不到——
		 * 这个弹窗补上那部分。
		 *
		 * 三条交互要求（缺一条都会被当成 bug）：Esc 关、点遮罩关、右上角 × 关。
		 * 打开时把焦点移到卡片上，关掉时还回原来那一行，键盘用户才不会「点开之后
		 * 焦点丢在页面开头」。
		 */
		function CommitDetailDialog({ t, state, onClose, onRetry }) {
			const cardRef = react.useRef(null);
			const [copied, setCopied] = react.useState(false);

			react.useEffect(() => {
				const onKey = (e) => { if (e.key === "Escape") { e.stopPropagation(); onClose(); } };
				// 挂 window 而不是卡片本身：焦点可能落在卡片内的按钮上，也可能因为
				// 异步渲染还没落下来，挂在窗口上才保证 Esc 一定生效。
			...
			const onCopy = react.useCallback(() => {
				if (!data) return;
				// clipboard 在非安全上下文/无权限时会 reject，复制不了就静默——这只是
				// 顺手功能，不该弹错误盖住用户真正要看的详情。
				try {
					const p = navigator.clipboard && navigator.clipboard.writeText(data.hash);
					if (p && typeof p.then === "function") p.then(() => setCopied(true), () => {});
					else setCopied(true);
				} catch (_) { /* 忽略 */ }
			}, [data]);

			return react_jsx_runtime.jsx("div", {
				className: "dsgDetailMask",
				// 只认落在遮罩自身上的点击；冒泡上来的（点在卡片里）不能关窗。
				onClick: (e) => { if (e.target === e.currentTarget) onClose(); },
				children: react_jsx_runtime.jsxs("div", {
					className: "dsgDetailCard",
					role: "dialog",
					"aria-modal": "true",
					"aria-label": t("git.detail.title"),
					tabIndex: -1,
					ref: cardRef,
					children: [
						react_jsx_runtime.jsxs("div", { className: "dsgDetailHead", children: [
							react_jsx_runtime.jsx("span", { className: "dsgDetailTitle", children: t("git.detail.title") }),
							react_jsx_runtime.jsx("button", {
								type: "button",
								className: "dsgDetailClose",
								onClick: onClose,
								title: t("git.detail.close"),
								"aria-label": t("git.detail.close"),
								children: "×"
							})
						] }),
						react_jsx_runtime.jsx("div", { className: "dsgDetailBody", children:
							state.status === "loading"
								? react_jsx_runtime.jsx("div", { className: "dsgDetailState", children: t("git.detail.loading") })
								: state.status === "error"
									? react_jsx_runtime.jsxs("div", { className: "dsgDetailState", children: [
										react_jsx_runtime.jsx("div", { children: state.message }),
										react_jsx_runtime.jsx("button", {
											type: "button",
											className: "dsgDetailCopy",
											style: { marginTop: "8px" },
											onClick: onRetry,
											children: t("git.detail.retry")
										})
									] })
									: react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, { children: [
										react_jsx_runtime.jsxs("div", { className: "dsgDetailMetaRow", children: [
											react_jsx_runtime.jsx("span", { className: "dsgDetailHash", children: data.shortHash }),
											react_jsx_runtime.jsx("button", {
												type: "button",
												className: "dsgDetailCopy",
												onClick: onCopy,
												title: t("git.detail.copy"),
												children: copied ? t("git.detail.copied") : t("git.detail.copy")
											}),
											react_jsx_runtime.jsxs("span", { children: [data.author, data.email ? ` <${data.email}>` : ""] }),
											react_jsx_runtime.jsx("span", { children: "·" }),
											react_jsx_runtime.jsx("span", { children: formatDate(data.date) })
										] }),
										react_jsx_runtime.jsx("div", { className: "dsgDetailSubTitle", children:
											react_jsx_runtime.jsx("span", { children: t("git.detail.message") })
										}),
										react_jsx_runtime.jsx("div", { className: "dsgDetailMsg", children:
											data.body ? data.subject + "\n\n" + data.body : data.subject
										}),
										react_jsx_runtime.jsxs("div", { className: "dsgDetailSubTitle", children: [
											react_jsx_runtime.jsx("span", { children: t("git.detail.files") }),
											react_jsx_runtime.jsxs("span", { className: "dsgDetailSubCount", children: [
												t("git.detail.summary").replace("{files}", String(data.totals.files)),
												"  ",
												react_jsx_runtime.jsx("span", { className: "dsgStatAdd", children: "+" + data.totals.insertions }),
												" ",
												react_jsx_runtime.jsx("span", { className: "dsgStatDel", children: "−" + data.totals.deletions })
											] })
										] }),
										data.files.length === 0
											? react_jsx_runtime.jsx("div", { className: "dsgDetailState", children: t("git.detail.noFiles") })
											: react_jsx_runtime.jsx("ul", { className: "dsgDetailFiles", children:
												data.files.map((f) => react_jsx_runtime.jsx(CommitFileRow, { t, file: f }, f.path))
											})
									] })
						})
					]
		...
		/** 已推送的提交至少露这么多条；下面还空着的话会自动再往下多铺几条。 */
		const PUSHED_PREVIEW = 5;

		function HistorySection({ t, commits, onUndo, undoing, unpushedCount = 0, onOpenDetail }) {
			const [showAllPushed, setShowAllPushed] = react.useState(false);
			// 面板高度是写死的 min(760px,88vh)：没有变更、历史又没展开时，下半截会
			// 空一大片。与其留白，不如让「最近提交」自己长高——量一下除历史行以外的
			// 东西占了多少，剩下的高度能放几行就多铺几行（见下面的 measure）。
			const [autoPreview, setAutoPreview] = react.useState(PUSHED_PREVIEW);
			const rowsRef = react.useRef(null);
			const sectionRef = react.useRef(null);
			// 「量→改→再量」的护栏：量到不用改就复位；真遇到来回跳的内容组合，
			// 最多跑几轮就停手，绝不能把面板拖进无限重渲染。
			const passesRef = react.useRef(0);
			// 未推送的全露出来——那是还等着你处理的；已推送的属于「已经了结」，
			// 只留最近几条，再多就收起来，免得历史把面板撑成一条长清单。
			const unpushedList = commits.slice(0, unpushedCount);
			const pushedList = commits.slice(unpushedCount);
			const previewCount = Math.min(autoPreview, Math.max(pushedList.length, PUSHED_PREVIEW));
			const previewPushed = pushedList.slice(0, previewCount);
			const restPushed = pushedList.slice(previewCount);
			const hiddenPushed = restPushed.length;

			// 展开态不重算：展开时该露的都露着，再动预览条数只会让预览区和折叠区
			// 来回搬家，展开/收起的动画跟着抖。
			const measure = () => {
				const rows = rowsRef.current, sec = sectionRef.current;
				if (!rows || !sec || showAllPushed) return;
				const body = sec.closest(".dsgBody");
				if (!body) return;
				const shown = unpushedList.length + previewPushed.length;
				const rowsH = rows.offsetHeight;
				if (!rowsH || shown <= 0) return;
				const rowH = rowsH / shown;
				// 「下面还空着多少」只能自己算：scrollHeight 至少等于 clientHeight，
				// 内容没撑满时它俩一样大，量不出空白。改用 offsetTop/offsetHeight
				// 推内容底边——两者都是布局值，既不受滚动位置影响，也不会被面板
				// 开合动画那个 scale(.98) 缩掉（getBoundingClientRect 会）。历史区是
				// body 的最后一块，它的下边加上自己的 margin 和 body 的 padding
				// 就是内容总高。
				const px = (v) => parseFloat(v) || 0;
				const contentH = sec.offsetTop + sec.offsetHeight - body.offsetTop
					+ px(getComputedStyle(sec).marginBottom) + px(getComputedStyle(body).paddingBottom);
				const gap = body.clientHeight - contentH;
				// 四舍五入到整行：空白**不足半行也补一条**，让它稍微溢出去——底部本来
				// 就压着渐隐，露半行反而是「下面还有」的提示，比空着小半行好看。要是
				// 只肯按整行补（floor），最多能剩下差一像素就够一行的空白，正是这版
				// 之前「填完还是空一截」的原因。两个方向共用一次 round，一轮之后余量
				// 必定不超过半行，不会「加一条又减一条」地来回跳。
				let next = previewCount + Math.round(gap / rowH);
				next = Math.min(Math.max(next, PUSHED_PREVIEW), Math.max(pushedList.length, PUSHED_PREVIEW));
				if (next === autoPreview) { passesRef.current = 0; return; }
				if (passesRef.current >= 6) return;
				passesRef.current += 1;
				setAutoPreview(next);
			};

			// 每次渲染后都量一遍：变更增删、提交框长高、折叠区开合都会挪动下面那块
			// 空白，而它们无一例外都伴随一次重渲染。用 layout effect 是为了赶在浏览器
			// 绘制前定好条数，不然会先闪一下短的再变长。
			react.useLayoutEffect(measure);

			// 折叠动画跑完、窗口改高之后再补一次——这两件事收尾时不会有新的渲染，
			// 上面那轮量到的还是动画中途的高度。只认折叠用的 grid-template-rows，
			// 别被 hover 那些背景/透明度过渡（都会冒泡到 body）带着空跑。
			react.useEffect(() => {
				const sec = sectionRef.current;
				const body = sec && sec.closest(".dsgBody");
				if (!body) return;
				const again = (e) => {
					if (e && e.type === "transitionend" && e.propertyName !== "grid-template-rows") return;
					passesRef.current = 0;
					measure();
				};
				body.addEventListener("transitionend", again);
				window.addEventListener("resize", again);
				return () => {
					body.removeEventListener("transitionend", again);
			...
			const renderRow = (c, index) => {
						...
						return react_jsx_runtime.jsxs("div", {
							className: "dsgHistoryRow",
							// 整行可点开详情。列表行只放得下 subject 一行，正文与文件清单
							// 都在弹窗里。role/tabIndex/onKeyDown 三件套缺一不可——只挂
							// onClick 的 div 键盘用户完全够不到。
							role: "button",
							tabIndex: 0,
							title: t("git.detail.openHint"),
							onClick: () => onOpenDetail && onOpenDetail(c),
							onKeyDown: (e) => {
								if (e.key !== "Enter" && e.key !== " ") return;
								e.preventDefault();
								if (onOpenDetail) onOpenDetail(c);
							},
							children: [
							react_jsx_runtime.jsxs("div", { className: "dsgHistoryTop", children: [
								...
								// 撤销按钮嵌在可点的行里，不拦冒泡的话点它会顺带弹出详情。
								index === 0 ? react_jsx_runtime.jsx("span", {
									onClick: (e) => e.stopPropagation(),
									children: react_jsx_runtime.jsx(UndoCommitButton, { t, undoing, onUndo })
								}) : null
							] }),
			...
			return react_jsx_runtime.jsxs("div", { className: "dsgSection", ref: sectionRef, children: [
				...
				commits.length === 0
					...
					: react_jsx_runtime.jsxs(react_jsx_runtime.Fragment, { children: [
						// 常显区：未推送的全部 + 前几条已推送。单独包一层是为了量它的高度，
						// 反推一行多高、下面还塞得下几行。
						react_jsx_runtime.jsxs("div", { ref: rowsRef, children: [
							unpushedList.map((c, i) => renderRow(c, i)),
							previewPushed.map((c, i) => renderRow(c, unpushedCount + i))
						] }),
						...
						hiddenPushed > 0 ? react_jsx_runtime.jsx("div", {
							...
							children: react_jsx_runtime.jsx("div", {
								...
								children: restPushed.map((c, i) => renderRow(c, unpushedCount + previewCount + i))
							})
						}) : null,
		...
		function GitPanelBody({ t, onClose, workspacesList, sessionsList, open }) {
			...
			// 提交详情弹窗：null = 关闭；否则 { commit, status, data|message }。
			// 每次打开都重新拉一次，不做缓存——提交是不可变的，但「改了又 amend」
			// 这类情况下缓存反而会显示旧内容，而这个请求本身很便宜。
			const [detail, setDetail] = react.useState(null);

			// 仓库发现：工作区根不是仓库时（一个工作区罩住 N 个仓库的布局），服务端扫一层
			// 子目录，这里给出「仓库」下拉。repoState.workspaceId 记的是「这份结果是哪个
			// 工作区的」——切工作区之后、新结果回来之前不能拿旧列表去发请求。
			const [repoState, setRepoState] = react.useState({ workspaceId: null, repos: [], suggested: null });
			const [repoId, setRepoId] = react.useState(null);
			// 手动刷新时要连仓库列表一起重扫：用户可能刚 git init 了一个新的子目录，
			// 或者刚把某个子仓库删掉。这个计数器就是「重扫一次」的信号。
			const [rescan, setRescan] = react.useState(0);

			react.useEffect(() => {
				if (!open || !selectedId) return undefined;
				let alive = true;
				getJson(`/api/dsdesktop/git/repos?workspaceId=${encodeURIComponent(selectedId)}`).then((result) => {
					if (!alive) return;
					// 失败不在这儿报：紧接着的 status 请求走的是同一条解析路径，会把同一个
					// 错误用同一套文案显示出来。这里只要别让面板卡在「等发现结果」上。
					const data = result && result.ok && result.data ? result.data : { repos: [], suggested: null };
					const repos = Array.isArray(data.repos) ? data.repos : [];
					setRepoState({ workspaceId: selectedId, repos, suggested: data.suggested ?? null });
					setRepoId(pickRepo(selectedId, repos, data.suggested ?? null));
				}).catch(() => {
					if (!alive) return;
					setRepoState({ workspaceId: selectedId, repos: [], suggested: null });
					setRepoId(null);
				});
				return () => { alive = false; };
			}, [open, selectedId, rescan]);

			// 这份仓库列表是不是当前工作区的。false 时 status/log/branches 一律先别发：
			// 带着上一个工作区的 repo（或者还没定下来的 null）去问，只会先闪一下
			// 「不是 Git 仓库」再被正确结果覆盖。
			const repoReady = repoState.workspaceId === selectedId;

			const onSelectRepo = react.useCallback((next) => {
				setRepoId(next);
				if (selectedId) storeRepo(selectedId, next);
			}, [selectedId]);

			const openDetail = react.useCallback((commit) => {
				setDetail({ commit, status: "loading" });
			}, []);
			const closeDetail = react.useCallback(() => setDetail(null), []);
			const retryDetail = react.useCallback(() => {
				setDetail((prev) => (prev ? { commit: prev.commit, status: "loading" } : prev));
			}, []);

			react.useEffect(() => {
				// 只在 loading 态发请求：ready/error 态重渲染不该再打一次。
				if (!detail || detail.status !== "loading" || !selectedId) return undefined;
				let alive = true;
				const hash = detail.commit.hash;
				const qs = `workspaceId=${encodeURIComponent(selectedId)}&hash=${encodeURIComponent(hash)}`
					+ (repoId ? `&repo=${encodeURIComponent(repoId)}` : "");
				getJson(`/api/dsdesktop/git/commit-detail?${qs}`).then((result) => {
					if (!alive) return;
					setDetail((prev) => {
						// 期间用户关掉了弹窗、或点开了另一条提交，这次的结果就作废。
						if (!prev || prev.commit.hash !== hash) return prev;
						return result && result.ok
							? { commit: prev.commit, status: "ready", data: result.data }
							: { commit: prev.commit, status: "error", message: (result && result.error && result.error.message) || "读取失败" };
					});
				}).catch((err) => {
					if (!alive) return;
					setDetail((prev) => (prev && prev.commit.hash === hash
						? { commit: prev.commit, status: "error", message: String(err && err.message ? err.message : err) }
						: prev));
				});
				return () => { alive = false; };
			}, [detail, selectedId, repoId]);

			// 切换工作区或仓库时把弹窗关掉：里面显示的是上一个仓库的提交。
			react.useEffect(() => { setDetail(null); }, [selectedId, repoId]);

			// repo 是可选的：工作区根自己就是仓库时它是 null，请求里干脆不带这个参数，
			// 行为跟单仓库时代一模一样。参数而不是从闭包里读，是为了别在「刚换完仓库
			// 就刷新」时读到上一轮的值。
			const refresh = react.useCallback(async (workspaceId, repo) => {
				if (!workspaceId) {
				...
				try {
					const qs = `workspaceId=${encodeURIComponent(workspaceId)}` + (repo ? `&repo=${encodeURIComponent(repo)}` : "");
					const [statusRes, logRes, branchesRes] = await Promise.all([
						getJson(`/api/dsdesktop/git/status?${qs}`),
						getJson(`/api/dsdesktop/git/log?${qs}&limit=30`),
						getJson(`/api/dsdesktop/git/branches?${qs}`)
					]);
					if (!statusRes.ok) {
			...
			react.useEffect(() => {
				if (!open) return;
				setActionError(null);
				// 选了工作区但仓库列表还没回来，就先等着（见 repoReady）。
				if (selectedId && !repoReady) return;
				refresh(selectedId, repoId);
			}, [open, selectedId, repoReady, repoId, rescan, refresh]);

			const runAction = react.useCallback(async (url, extra) => {
				if (!selectedId) return;
				setActionError(null);
				try {
					const result = await postJson(url, { workspaceId: selectedId, repo: repoId, ...extra });
					if (!result.ok) {
						setActionError(errorMessage(t, result.error));
						return;
					}
					await refresh(selectedId, repoId);
				} catch (error) {
					setActionError(String(error?.message ?? error));
				}
			}, [selectedId, repoId, refresh, t]);

			const onStage = (path) => runAction("/api/dsdesktop/git/stage", { paths: [path] });
			const onUnstage = (path) => runAction("/api/dsdesktop/git/unstage", { paths: [path] });
			...
			const onCommit = react.useCallback(async () => {
				...
				setActionError(null);
				try {
					const result = await postJson("/api/dsdesktop/git/commit", { workspaceId: selectedId, repo: repoId, message: message.trim() });
					if (result.ok) {
						setMessage("");
			...
			}, [selectedId, repoId, message, refresh, t]);
			...
			const onPush = react.useCallback(async () => {
				...
				setActionError(null);
				try {
					const result = await postJson("/api/dsdesktop/git/push", { workspaceId: selectedId, repo: repoId });
					if (result.ok) {
			...
			}, [selectedId, repoId, pushing, refresh, t]);
			...
			const onUndoCommit = react.useCallback(async () => {
				...
				setActionError(null);
				try {
					const result = await postJson("/api/dsdesktop/git/undo-commit", { workspaceId: selectedId, repo: repoId });
					if (result.ok) {
			...
			}, [selectedId, repoId, undoing, refresh, t]);
			...
			const onCheckout = react.useCallback(async (branch) => {
				...
				setActionError(null);
				try {
					const result = await postJson("/api/dsdesktop/git/checkout", { workspaceId: selectedId, repo: repoId, branch });
					if (result.ok) {
						await refresh(selectedId, repoId);
					} else {
						setActionError(errorMessage(t, result.error));
			...
			}, [selectedId, repoId, switchingBranch, refresh, t]);
			...
			return react_jsx_runtime.jsxs("div", { className: "dsgPanel" + (open ? " dsgOpen" : ""), children: [
				react_jsx_runtime.jsxs("div", { className: "dsgHeader", children: [
					...
					react_jsx_runtime.jsx("button", {
						...
						// 只需要拨一下 rescan：上面两个 effect 都盯着它，仓库列表与面板数据
						// 会各自重新拉一遍，不用在这里手写两次请求。
				...
				react_jsx_runtime.jsxs("div", { className: "dsgBody", children: [
					...
					// 仓库下拉只在「工作区根自己不是仓库、但下面扫到了仓库」时出现。单仓库
					// 用户看不到它，面板跟以前一模一样——多一个永远只有一个选项的下拉是纯噪音。
					repoState.repos.length > 0 ? react_jsx_runtime.jsxs("div", { className: "dsgWorkspaceRow", children: [
						react_jsx_runtime.jsx("label", { className: "dsgSectionTitle", children: t("git.repo.label") }),
						react_jsx_runtime.jsx(Dropdown, {
							ariaLabel: t("git.repo.label"),
							value: repoId || "",
							onChange: onSelectRepo,
							options: repoState.repos.map((repo) => ({ value: repo.id, label: repo.label }))
						})
					] }) : null,
					...
					// 「不是仓库」而且一层子目录里也一个都没扫到时补一句：否则用户不知道
					// 我们到底找没找过、找到哪一层，只能对着一句否定发呆。
					selectedId && view.status === "error" && view.error && view.error.code === "not-a-repo" && repoReady && repoState.repos.length === 0
						? react_jsx_runtime.jsx("div", { className: "dsgEmpty", children: t("git.repo.none") }) : null,
				...
				// 弹窗写在最后：同层里后写的盖在上面，不用给它再堆 z-index 去压
				// 前面的渐隐层与滚动内容。
				detail ? react_jsx_runtime.jsx(CommitDetailDialog, {
					t, state: detail, onClose: closeDetail, onRetry: retryDetail
				}) : null
			] });
		}
		...
		function GitPanel({ t, store, workspacesList, sessionsList }) {
			...
			// createPortal：槽位仍然是 shell.overlay（生命周期跟着插件走），
			// 但真正的 DOM 挂到 document.body —— 只有跳出 .overlayLayer{z-index:20} 这个
			// 层叠上下文，遮罩的 1000 才有意义（理由见 .dsgBackdrop 的注释）。
			// 这也是 dsh 自己弹窗的做法（dsh-client-ui-attachment 的图片灯箱同款）。
		...
		exports.__test__ = { CommitBox, HistorySection, CommitDetailDialog, CommitFileRow };
