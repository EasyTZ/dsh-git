window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function GitPanelBody({ t, onClose, workspacesList, sessionsList, open }) {
			...
			// 切换工作区时把弹窗关掉：里面显示的是上一个仓库的提交。
			react.useEffect(() => { setDetail(null); }, [selectedId]);
			...
			const onCommit = react.useCallback(async () => {
				...
				setActionError(null);
				try {
					const result = await postJson("/api/dsdesktop/git/commit", { workspaceId: selectedId, message: message.trim() });
					if (result.ok) {
						setMessage("");
			...
			const onPush = react.useCallback(async () => {
				...
				setActionError(null);
				try {
					const result = await postJson("/api/dsdesktop/git/push", { workspaceId: selectedId });
					if (result.ok) {
			...
			const onUndoCommit = react.useCallback(async () => {
				...
				setActionError(null);
				try {
					const result = await postJson("/api/dsdesktop/git/undo-commit", { workspaceId: selectedId });
					if (result.ok) {
			...
			const onCheckout = react.useCallback(async (branch) => {
				...
				setActionError(null);
				try {
					const result = await postJson("/api/dsdesktop/git/checkout", { workspaceId: selectedId, branch });
					if (result.ok) {
						await refresh(selectedId);
					} else {
						setActionError(errorMessage(t, result.error));
